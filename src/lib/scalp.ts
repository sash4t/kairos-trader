...
export const STRATEGY_OPTIONS = [
  ...
  { key: RSI_EXTREMES_KEY, name: "1H RSI Extremes", description: "1H RSI(14) mean-reversion: scan every eligible liquid market each minute, buy reversals from oversold ≤30, short reversals from overbought ≥70, ride toward the 50 zone." },
] as const;
...
export function strategySelectionPatch(key: StrategyKey): Record<string, unknown> {
  ...
  if (key === RSI_EXTREMES_KEY) return { strategy_key: key, min_confidence: 70, trailing_enabled: false, max_positions: 5, daily_loss_pct: 5 };
